...
test_that("a data frame with no missing values", {
  ...
  expectedNA <- setNames(c(0, 0, 0), c("a", "b", "c"))
  expectedUnique <- setNames(c(3, 3, 3), c("a", "b", "c"))

  expect_type(results, "list")
  expect_s3_class(results, "MissingValues")
  expect_length(results, 2)
  expect_identical(trunc(results$naResults), expectedNA)
  expect_identical(trunc(results$uniqueResults), expectedUnique)
})
...
test_that("a data frame with multiple missing values", {
  ...
  expectedNA <- setNames(c(2, 0, 3), c("a", "b", "c"))
  expectedUnique <- setNames(c(4, 5, 3), c("a", "b", "c"))
...
test_that("a data frame with repeating values and missing values", {
  ...
  expectedNA <- setNames(c(1, 0, 5), c("a", "b", "c"))
  expectedUnique <- setNames(c(2, 3, 1), c("a", "b", "c"))
