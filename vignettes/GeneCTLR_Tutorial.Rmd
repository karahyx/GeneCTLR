...
newRBPs <- subset(imputedRBPs, select = -c(1, 8))

# Model fitting
# The autism_genes variable is the 10th column in newRBPs, so colIndex = 10.
results <- trainCV(data = newRBPs, colIndex = 10, K = 5)
results$models
```
...
plotPR(results$predictions[[1]], results$testSets[[1]]$autism_genes)
```
...
```{r, echo = TRUE}
predictions <- addPredictions(results, imputedRBPs)

# We can sort the genes by highest to lowest predictions
predictions <- predictions %>% arrange(-predictions)
